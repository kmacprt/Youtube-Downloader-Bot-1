    ...
    if cb_data.startswith("ytdata||"):
        ...
        if media_type == 'audio':
            buttons = InlineKeyboardMarkup([[InlineKeyboardButton(
                "ශව්‍යය 🔈", callback_data=f"{media_type}||{format_id}||{yturl}"), InlineKeyboardButton("ලේඛනය📑",
...
async def catch_youtube_dldata(c, q):
    ...
    await q.edit_message_reply_markup(
        InlineKeyboardMarkup([[InlineKeyboardButton("බාගත වීම සිදු වේ...📥", callback_data="down")]]))
...
async def send_file(c, q, med, filename):
    ...
    try:
        await q.edit_message_reply_markup(
            InlineKeyboardMarkup([[InlineKeyboardButton("උඩුගත විම සිදු වේ...📤", callback_data="down")]]))
